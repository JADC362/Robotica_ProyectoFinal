...
#Constante del sistema de control -  np.array([0.11,0.09,0.01])*2.8;
k = np.array([0.11,0.09,0.01])*2.8;
...
def callbackGeneralPositions(msg):
	...
	#for i in range(numeroCeldas):
		#obstaculos.append([i*tamanoGrilla,0,0]);
		#obstaculos.append([i*tamanoGrilla,tamanoMapa-tamanoGrilla,0]);
...
def determinarObstaculoGrilla(obstaculo,posicionGrilla):
	...
	if distanciaObsGrilla <= ((obstaculo[2]/2.0)+razonObstaculoError):
	...
	for m in range(numeroCeldas):
		...
		for n in range(numeroCeldas):
			...
			if (i > 0 and j > 0) and (i < numeroCeldas-1 and j < numeroCeldas-1):
				...
				G.add_edge("{},{}".format(j,i),"{},{}".format(j+1,i),weight=pesoI)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j-1,i),weight=pesoA)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j,i+1),weight=pesoDe)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j,i-1),weight=pesoIz)

				G.add_edge("{},{}".format(j,i),"{},{}".format(j+1,i+1),weight=pesoEsqID)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j-1,i-1),weight=pesoEsqSI)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j-1,i+1),weight=pesoEsqSD)
				G.add_edge("{},{}".format(j,i),"{},{}".format(j+1,i-1),weight=pesoEsqII)
	#nx.draw_spectral(G);
	pathPlaning();
...
def calcularCinematicaRobot(puntoFinal):
	...
	posPol = obtenerPosicionPol(puntoFinal)
	print("Error 1:{}".format(posPol));
	posPol = np.array([0,0,puntoFinal[2]])-posPol
	print("Error 2:{}".format(posPol));
